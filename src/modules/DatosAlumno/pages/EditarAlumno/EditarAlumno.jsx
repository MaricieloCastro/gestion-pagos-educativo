...
const EditarAlumno = () => {
    ...
    try {
      ...
      data.forEach((item) => {
        if (item.alumno) {
          ...
          setFotografia(item.alumno.ruta_fotografia)
        }
        if (item.familiar) {
  ...
  const onSubmit = async () => {
    try {
      const estudianteData = convertValuesToUpperCase(formDataEstudiante)
      const padreData = convertValuesToUpperCase(formDataPadre)
      const madreData = convertValuesToUpperCase(formDataMadre)

      console.log('foto', fotografia)
      console.log('estudianteData', estudianteData)

      if (fotografia) {
        estudianteData.ruta_fotografia = renameFile(
          fotografia,
          estudianteData.dni
        )
      }
