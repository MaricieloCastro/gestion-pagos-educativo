...
const ListasTable = (props) => {
  ...
  const numItems = numItemsForPage();
    ...
    <div className='grid '>
      <div className='flex items-center bg-gray-listas h-10 border-b-2  border-b-white-linea'>
        <h1 className='mx-5 text-white font-inter'>
          MOSTRANDO {numItems} DE {totalItems}° REGISTROS
        </h1>
      </div>
      {loading ? (
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      for (let key in rowsSelect) {
                        newIds.push(...ids, {
                          id: table.getRowModel().rows[rowsSelect[key]].original
                            .id
                        });
                      }
...
ListasTable.propTypes = {
  numItemsForPage: PropTypes.func,
